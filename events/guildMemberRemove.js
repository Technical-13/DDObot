...
client.on( 'guildMemberRemove', async ( member ) => {
  try {
    ...
    if ( ndxUserGuild != -1 ) {
      ...
      .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( 'Error attempting to update guild %s (id: %s) for user %s (id: %s) to expire %o in my database in guildMemberRemove.js:\n%o' ), guild.name, guild.id, user.displayName, user.id, dbExpires, updateError ); } );
    }
    ...
    .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( 'Error attempting to update %s (id: %s) in my database:\n%o' ), guild.name, guild.id, updateError ); } );
  }
