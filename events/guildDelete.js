...
client.on( 'guildDelete', async ( guild ) => {
  try {
    ...
    .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( `Error attempting to update ${guild.name} (id: ${guild.id}) to expire in DB:\n${dbExpires}\nError:\n${updateError}` ) ); } );
    ...
    memberIds.forEach( async ( memberId ) => {// Update users for this guild to expire.
      ...
      if ( ndxUserGuild != -1 ) {
        ...
        .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( 'Error attempting to update guild %s (id: %s) for user %s (id: %s) to expire %o in my database in guildDelete.js:\n%o' ), guild.name, guild.id, currUser.UserName, userId, dbExpires, updateError ); } );
      }
    } );
  }
