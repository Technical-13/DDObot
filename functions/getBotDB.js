  ...
  try {
    ...
    if ( !newBot ) {
      ...
      if ( configVersion === verBotDB ) { return currConfig; }
        ...
        .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( 'Error attempting to update bot config in my database:\n%o' ), updateError ); } );
      }
    }
      ...
      .catch( initError => { throw new Error( chalk.cyan.inverse.bold( `Error attempting to initialize bot configuration in my database:\n${initError}` ) ); } );
    }
  }
