...
module.exports = async ( id, guild ) => {
  ...
  try {
    ...
    .catch( updateError => { throw new Error( chalk.cyan.inverse.bold( `Error attempting to add guild ${guild.name} (id: ${guild.id}) to user ${user.displayName} (id: ${id}) in my database in addUserGuild.js:\n${updateError}` ) ); } );
  }
